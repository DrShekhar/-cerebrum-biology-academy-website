        ...
        >
          <ol className="flex flex-wrap items-center gap-1">
            {allItems.map((item, index) => (
              <li
                key={`breadcrumb-${item.label.toLowerCase().replace(/\s+/g, '-')}-${item.href || 'current'}`}
      ...
      <>
        <nav aria-label="Breadcrumb" className={`breadcrumb-pills ${className}`}>
          <ol className="flex flex-wrap items-center gap-2">
            {allItems.map((item, index) => (
              <li
                key={`breadcrumb-pill-${item.label.toLowerCase().replace(/\s+/g, '-')}-${item.href || 'current'}`}
                className="flex items-center"
              >
    ...
    <>
      <nav aria-label="Breadcrumb" className={`breadcrumb-default ${className}`}>
        <ol className="flex flex-wrap items-center text-sm">
          {allItems.map((item, index) => (
            <li
              key={`breadcrumb-default-${item.label.toLowerCase().replace(/\s+/g, '-')}-${item.href || 'current'}`}
              className="flex items-center"
            >
