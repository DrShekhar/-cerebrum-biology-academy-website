...
interface StatementBasedCardProps {
...
function parseStatements(question: MCQQuestion): {
  prompt: string
  statements: string[]
  choices: string[]
} {
  ...
  for (const line of lines) {
    if (/^(I{1,4}|[ivxIVX]+)[.)]\s/.test(line) || /^\d+[.)]\s/.test(line)) {
  ...
  if (statements.length === 0) {
    const stmtPattern = /(?:^|\n)\s*((?:I{1,4}|[ivxIVX]+)[.)]\s*.+)/gm
    let m
    while ((m = stmtPattern.exec(text)) !== null) {
      statements.push(m[1].trim())
    }
  }

  if (!prompt) {
    prompt = 'Which of the following statements is/are correct?'
  }

  return { prompt, statements, choices: question.options }
}

export function StatementBasedCard({
  question,
  questionNumber,
  onAnswer,
  ...
  const { prompt, statements, choices } = parseStatements(question)
        ...
        {statements.length > 0 && (
          <div className="space-y-2 pl-1">
            {statements.map((stmt, i) => (
              <div
                key={i}
                className="flex items-start gap-2 p-2.5 rounded-lg border-l-3 border-purple-300 bg-purple-50/40"
              >
                <p className="text-base text-ink leading-relaxed">{stmt}</p>
              </div>
            ))}
          </div>
        )}
