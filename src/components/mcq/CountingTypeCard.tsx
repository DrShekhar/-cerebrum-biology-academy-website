...
interface CountingTypeCardProps {
...
function parseCountingItems(question: MCQQuestion): {
  prompt: string
  items: string[]
  choices: string[]
} {
  ...
  if (!prompt) {
    prompt = 'How many of the following are correct?'
  }

  return { prompt, items, choices: question.options }
}

export function CountingTypeCard({
  question,
  questionNumber,
  onAnswer,
  ...
  const { prompt, items, choices } = parseCountingItems(question)
    ...
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-yellow-700 flex items-center gap-1">
      ...
      <div className="mb-4">
        ...
        {items.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-3 rounded-lg bg-amber-50/40 border border-amber-200/50">
            {items.map((item, i) => (
              <div key={i} className="flex items-start gap-2 py-1">
                <span className="flex-shrink-0 w-5 h-5 rounded-full bg-amber-200 text-yellow-800 flex items-center justify-center text-xs font-bold mt-0.5">
                  {i + 1}
                </span>
                <p className="text-sm text-ink leading-relaxed">
                  {item.replace(/^[\d]+[.)]\s|^[-•]\s|^[a-h][.)]\s/i, '')}
                </p>
              </div>
            ))}
          </div>
