...
interface MatchFollowingCardProps {
...
function parseMatchColumns(question: MCQQuestion): {
  prompt: string
  columnA: string[]
  columnB: string[]
  choices: string[]
  ...
  for (const line of lines) {
    if (/^[A-D][.)]\s/.test(line)) {
      columnA.push(line)
    } else if (/^[ivxIVX]+[.)]\s/.test(line) || /^[i-iv]+[.)]\s/.test(line)) {
      columnB.push(line)
    } else if (/^\d+[.)]\s/.test(line)) {
      columnB.push(line)
    } else if (columnA.length === 0 && columnB.length === 0) {
  ...
  return { prompt, columnA, columnB, choices: question.options }
}
...
export function MatchFollowingCard({
  ...
  const { prompt, columnA, columnB, choices } = parseMatchColumns(question)
  ...
  const maxRows = Math.max(columnA.length, columnB.length)
      ...
      {columnA.length > 0 && columnB.length > 0 ? (
        <div className="mb-4 overflow-x-auto">
          <table className="w-full border-collapse hidden md:table">
            <thead>
              <tr>
                <th className="text-left text-xs font-semibold text-teal-700 uppercase tracking-wide p-2 bg-teal-50/50 rounded-tl-lg border-b border-teal-200">
                  Column A
                </th>
                <th className="text-left text-xs font-semibold text-blue-700 uppercase tracking-wide p-2 bg-blue-50/50 rounded-tr-lg border-b border-blue-200">
                  Column B
                </th>
              </tr>
            </thead>
            <tbody>
              {Array.from({ length: maxRows }).map((_, i) => (
                <tr key={i} className={i % 2 === 0 ? 'bg-stone-50/30' : ''}>
                  <td className="p-2.5 text-sm text-ink border-b border-stone-100 bg-teal-50/20">
                    {columnA[i] || ''}
                  </td>
                  <td className="p-2.5 text-sm text-ink border-b border-stone-100 bg-blue-50/20">
                    {columnB[i] || ''}
                  </td>
                </tr>
              ))}
        ...
        <div className="mb-4">
          <p className="text-base text-ink leading-relaxed">{question.question}</p>
        </div>
      )}
