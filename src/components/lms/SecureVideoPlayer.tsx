...
/** Imperative handle so a parent (e.g. a timestamped notes list) can seek the
 *  underlying <video>. For YouTube-embed lectures the CF player isn't mounted,
 *  so callers simply hold no handle and seeking is a no-op. */
export interface SecureVideoPlayerHandle {
  /** Seek the video to `seconds`, resume playback, and scroll it into view. */
...
const SecureVideoPlayer = forwardRef<SecureVideoPlayerHandle, SecureVideoPlayerProps>(
  function SecureVideoPlayer(
    ...
    // External-seek entry point for parents (e.g. clicking a timestamped note).
    // Safe no-op when the <video> isn't mounted (YouTube-embed lectures don't
    // render this player at all, so their parent never holds a live handle).
    useImperativeHandle(
      ref,
      () => ({
