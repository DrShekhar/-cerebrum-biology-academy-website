        ...
        {[1, 2, 3].map((index) => (
          <div key={index} className="course-card loading">
            <div className="h-96 flex items-center justify-center">
              <div className="w-8 h-8 border-2 border-blue-500 border-t-transparent rounded-full animate-fadeInUp" />
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className={`course-series-grid animate-in ${className}`}>
      {seriesData.map((series, index) => (
        <div key={`${series.id}-${selectedClass}`} className={getSeriesClassName(series.id)}>
          {/* Main Series Card */}
          <SeriesCard series={series} classLevel={selectedClass} onPlanSelect={handlePlanSelect} />

          {/* Enhanced Plan Buttons Overlay */}
          {hoveredCard === series.id && (
            <div className="plan-buttons-container animate-fadeInUp">
              {/* Quick Plan Selection */}
              <div className="space-y-3">
                <h4 className="text-white font-bold text-lg mb-4 text-center animate-fadeInUp">
                  Quick Plan Selection
                </h4>
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="plan-button-details">
                        ...
                        <div className="plan-button-subtitle">
                          {plan.duration}
                          {plan.popular && <span className="ml-2 text-yellow-300">⭐ Popular</span>}
                        </div>
                      </div>
                    </div>
                ...
                {/* Action Buttons */}
                <div className="flex space-x-3 mt-4 animate-fadeInUp">
                  <button className="flex-1 bg-white/20 backdrop-blur-sm text-white font-semibold py-2 px-4 rounded-lg border border-white/30 hover:bg-white/30 transition-colors animate-fadeInUp">
                    View Details
                  </button>
                  {selectedPlans[series.id] && (
                    <button className="flex-1 bg-[#4a5d4a] text-white font-bold py-2 px-4 rounded-lg shadow-lg hover:shadow-xl transition-shadow animate-fadeInUp">
                      Enroll Now
                    </button>
                  )}
                </div>
                ...
                {/* Quick Info */}
                <div className="text-center text-white/70 text-sm mt-3 animate-fadeInUp">
      ...
      {Object.keys(selectedPlans).length > 0 && (
        <div className="col-span-full mt-8 animate-fadeInUp">
          <div className="bg-gradient-to-r from-green-50 to-green-50 rounded-3xl p-6 border border-green-200 backdrop-blur-sm">
            ...
            <div className="text-center mt-6 animate-fadeInUp">
              <button className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-8 rounded-2xl shadow-xl hover:shadow-2xl transition-all animate-fadeInUp">
