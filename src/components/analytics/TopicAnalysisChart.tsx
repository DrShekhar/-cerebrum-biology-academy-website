          ...
          <div key={topic.topic} className="space-y-2">
            ...
            <div className="relative">
              ...
              <div className="flex justify-between mt-1 text-xs text-gray-500">
                <span>
                  Easy: {topic.difficulty.easy.total > 0
                    ? Math.round((topic.difficulty.easy.correct / topic.difficulty.easy.total) * 100)
                    : 0}%
                </span>
                <span>
                  Medium: {topic.difficulty.medium.total > 0
                    ? Math.round((topic.difficulty.medium.correct / topic.difficulty.medium.total) * 100)
                    : 0}%
                </span>
                <span>
                  Hard: {topic.difficulty.hard.total > 0
                    ? Math.round((topic.difficulty.hard.correct / topic.difficulty.hard.total) * 100)
                    : 0}%
                </span>
              </div>
            </div>
          ...
          <svg className="w-full h-full max-w-xs" viewBox="0 0 200 200">
            ...
            {data.length > 0 && (
              <polygon
                ...
                stroke="#3b82f6"
                strokeWidth="2"
                points={data.slice(0, 6).map((topic, index) => {
          ...
          {data.map(topic => (
            <div key={topic.topic} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <div className="flex items-center gap-4 text-sm text-gray-600">
                <span>
                  Avg Time: {Math.round(topic.averageTime)}s
                </span>
