...
export const advancedAssertionReasonQuestions: AssertionReasonQuestion[] = [
  {
    ...
    explanation:
      'Mitochondria are indeed called the powerhouse of the cell because they generate most of the ATP required for cellular processes. The cristae (inner membrane folds) contain the electron transport chain where ATP is produced through oxidative phosphorylation.',
    ...
    timeManagementTip:
      'First verify if both statements are factually correct, then check causal relationship',
  },
  {
    id: 'ar-bio-002',
    chapterId: 'ch11-19',
    ...
    explanation:
      'DNA replication in prokaryotes is indeed bidirectional, proceeding in both directions from the origin. However, prokaryotes typically have only one origin of replication (oriC), unlike eukaryotes which have multiple origins.',
...
export const advancedMatchFollowingQuestions: MatchFollowingQuestion[] = [
  {
    ...
    explanation:
      'Simple diffusion occurs directly through the lipid bilayer. Facilitated diffusion uses channel/carrier proteins without energy. Active transport requires both energy (ATP) and carrier proteins. Osmosis is specifically water movement through semi-permeable membranes.',
  ...
  },
  {
    ...
    explanation:
      'Anther produces pollen grains containing male gametes. Stigma is the receptive surface for pollen. Ovary contains ovules which develop into seeds. Pollen tube grows from pollen grain to conduct sperm cells to the ovule.',
...
export const advancedDiagramBasedQuestions: DiagramBasedQuestion[] = [
  {
    ...
    questionText:
      'The diagram shows the structure of human respiratory system. Identify the part labeled X and its primary function:',
    questionType: 'diagram-based',
    difficulty: 'medium',
    diagramUrl: '/images/respiratory-system-diagram.png',
    diagramDescription:
      'Human respiratory system showing trachea, bronchi, bronchioles, and alveoli with X pointing to alveolar sacs',
    labelledParts: [
    ...
    explanation:
      'X represents the alveoli, which are tiny air sacs surrounded by dense capillary networks. This is where actual gas exchange occurs - oxygen enters the blood and carbon dioxide is removed. The thin alveolar wall facilitates efficient diffusion.',
  ...
  },
  {
    ...
    topicId: 'topic-cell-biology',
    questionText:
      'The electron micrograph shows a cell organelle. What is the function of the structure indicated by the arrow?',
    questionType: 'diagram-based',
    difficulty: 'hard',
    diagramUrl: '/images/chloroplast-structure.png',
    diagramDescription:
      'Chloroplast ultrastructure showing outer membrane, inner membrane, stroma, and thylakoids with arrow pointing to grana',
    labelledParts: [
    ...
    explanation:
      'The arrow points to grana, which are stacks of thylakoids. Grana contain chlorophyll and other photosynthetic pigments, and this is where light-dependent reactions (photosystem I and II) occur during photosynthesis.',
...
export const advancedMultipleCorrectQuestions: MultipleCorrectQuestion[] = [
  {
    ...
    explanation:
      'Enzymes lower activation energy (correct), are affected by temperature and pH (correct), and show substrate specificity (correct). However, not all enzymes are proteins (ribozymes are RNA), and enzymes are not consumed - they are regenerated after the reaction.',
  ...
  },
  {
    ...
    questionText:
      "In Mendel's experiments with pea plants, which of the following observations led to his laws of inheritance?",
    questionType: 'multiple-correct',
    difficulty: 'easy',
    options: [
    ...
    explanation:
      "Mendel's observations included: existence of alleles, dominance relationships, independent assortment, and the 3:1 ratio in F2. However, inheritance of acquired characteristics was proposed by Lamarck, not Mendel.",
...
export const advancedNumericalQuestions: NumericalQuestion[] = [
  {
    ...
    questionText:
      'During aerobic respiration, if 6 molecules of glucose are completely oxidized, how many molecules of ATP are produced through oxidative phosphorylation (assume 2.5 ATP per NADH and 1.5 ATP per FADH2)?',
    ...
    explanation:
      'Each glucose yields 8 NADH (2 from glycolysis + 6 from Krebs cycle) and 2 FADH2 (from Krebs cycle). In oxidative phosphorylation: 8 NADH × 2.5 ATP + 2 FADH2 × 1.5 ATP = 23 ATP per glucose. For 6 glucose: 6 × 23 = 138 ATP. Note: This excludes substrate-level phosphorylation.',
  ...
  },
  {
    ...
    topicId: 'topic-genetics',
    questionText:
      'In a dihybrid cross between two heterozygous parents (AaBb × AaBb), how many different genotypes are possible in the F2 generation?',
    questionType: 'numerical',
    difficulty: 'medium',
    answerType: 'integer',
    correctAnswer: 9,
    ...
    explanation:
      'In a dihybrid cross, each gene can have 3 genotypic forms (homozygous dominant, heterozygous, homozygous recessive). Since the genes assort independently, the total number of different genotypes is 3 × 3 = 9.',
    ...
    timeManagementTip:
      'Use the formula: (number of genotypes for gene 1) × (number of genotypes for gene 2)',
  },
]
...
export const advancedStatementBasedQuestions: StatementBasedQuestion[] = [
  ...
  },
  {
    ...
    explanation:
      'Statement 1 is correct - homologous organs show structural similarity due to common ancestry. Statement 2 is correct - vestigial organs are evolutionary remnants. Statement 3 is incorrect - analogous organs indicate convergent evolution, not common ancestry. Statement 4 is correct - embryological similarities reflect evolutionary relationships.',
...
export const advancedQuestionStats = {
  ...
  byDifficulty: {
    easy: Object.values(allAdvancedQuestions)
      .flat()
      .filter((q) => q.difficulty === 'easy').length,
    medium: Object.values(allAdvancedQuestions)
      .flat()
      .filter((q) => q.difficulty === 'medium').length,
    hard: Object.values(allAdvancedQuestions)
      .flat()
      .filter((q) => q.difficulty === 'hard').length,
  },
  avgTimeSeconds: Math.round(
    Object.values(allAdvancedQuestions)
      .flat()
