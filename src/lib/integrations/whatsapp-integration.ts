...
export class WhatsAppBusinessAPI {
  ...
  private async processMessage(message: WhatsAppMessage): Promise<{ success: boolean; messageId?: string; error?: string }> {
    try {
      const payload = {
  ...
  private async updateMessageStatus(whatsappId: string, status: string, timestamp: string): Promise<void> {
  ...
  // Template Management
  async createTemplate(template: Omit<WhatsAppTemplate, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
  ...
  class_reminder: {
    ...
    components: [
      ...
      },
      {
        type: 'buttons' as const,
        buttons: [
          { type: 'url' as const, text: 'Join Class', url: '{{4}}' }
        ]
      }
    ]
  },
