...
export async function criticalQualityRequest() {
  const request: AIRequest = {
    prompt:
      'A student is struggling with understanding DNA replication. Create a comprehensive explanation with analogies, step-by-step process, and memory techniques specifically for NEET preparation.',
    context: {
      subject: 'biology',
  ...
  if (response.success && response.metadata.qualityReport) {
    ...
    console.log(
      '  Educational Value:',
      response.metadata.qualityReport.metrics.educationalValue.toFixed(2)
    )
    console.log(
      '  NEET Alignment:',
      response.metadata.qualityReport.metrics.neetAlignment.toFixed(2)
    )
...
export async function systemStatusCheck() {
  ...
  console.log(
    '  Efficiency Score:',
    `${(costDashboard.optimization.efficiencyScore * 100).toFixed(1)}%`
  )
  ...
  if (response.success) {
    ...
    if (response.metadata.fallbackAttempts) {
      ...
      response.metadata.fallbackAttempts.forEach((attempt, i) => {
        console.log(
          `  ${i + 1}. ${attempt.provider}: ${attempt.success ? '✅' : '❌'} (${attempt.responseTime}ms)`
        )
      })
    }
  }
