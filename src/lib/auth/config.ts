 ...
 */
export function requireAuth(handler: (request: NextRequest, session: UserSession) => Promise<Response>) {
 ...
 */
export function optionalAuth(handler: (request: NextRequest, session?: UserSession) => Promise<Response>) {
  return async (request: NextRequest): Promise<Response> => {
    const session = await validateUserSession(request)
...
export class SessionManager {
  static async createSession(user: User): Promise<{ accessToken: string; refreshToken: string; sessionId: string }> {
...
export class AuthRateLimit {
  ...
  static checkRateLimit(identifier: string): { allowed: boolean; remainingAttempts?: number; lockoutEndsAt?: Date } {
    const now = Date.now()
    ...
    if (now - record.lastAttempt > this.LOCKOUT_DURATION || now - record.lastAttempt > this.ATTEMPT_WINDOW) {
      this.attempts.set(identifier, { count: 1, lastAttempt: now })
      return { allowed: true, remainingAttempts: this.MAX_ATTEMPTS - 1 }
    }
