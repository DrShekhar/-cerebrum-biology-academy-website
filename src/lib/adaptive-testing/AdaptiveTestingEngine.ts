...
class AdaptiveTestingEngine {
  ...
  getRealTimeAnalytics(sessionId: string): {
    ...
    return {
      ...
      adaptations: {
        total: Object.values(session.adaptations).reduce(
      ...
      engagementStatus:
        session.progressMetrics.engagementLevel > 0.7
          ...
          : session.progressMetrics.engagementLevel > 0.4
            ? 'moderate'
            : 'low',
      adaptationCount: Object.values(session.adaptations).reduce(
        (sum, val) => (typeof val === 'number' ? sum + val : sum),
        0
      ),
  ...
  private generateTestResult(session: AdaptiveTestSession): AdaptiveTestResult {
    ...
    return {
      ...
      adaptations: {
        totalAdjustments: Object.values(session.adaptations).reduce(
          (sum, val) => (typeof val === 'number' ? sum + val : sum),
          0
        ),
      ...
      diagnostics: {
        ...
        systemMetrics: {
          ...
          systemAdaptations: Object.values(session.adaptations).reduce(
            (sum, val) => (typeof val === 'number' ? sum + val : sum),
            0
          ),
        },
  ...
  private calculateAdaptationEffectiveness(session: AdaptiveTestSession): number {
    ...
    const totalAdaptations = Object.values(session.adaptations).reduce(
      (sum, val) => (typeof val === 'number' ? sum + val : sum),
      0
    )
