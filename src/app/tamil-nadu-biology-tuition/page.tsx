...
const WHATSAPP_MESSAGE = encodeURIComponent('Hi! I am interested in Tamil Nadu State Board Biology tuition. Please share details.')
...
const features = [
  { icon: Video, title: 'Live Interactive Classes', description: 'Real-time online coaching aligned with Tamil Nadu State Board syllabus. English medium focus.' },
  { icon: Users, title: 'Small Batches (10-15)', description: 'Personalized attention for every TN board student. Doubt clearing until mastery.' },
  { icon: Award, title: 'AIIMS Trained Faculty', description: 'Expert teachers who understand both TN pattern and NEET requirements.' },
  { icon: BookOpen, title: 'TN Board + NEET Dual Focus', description: 'Comprehensive preparation covering both Tamil Nadu board and NEET biology.' },
  { icon: Clock, title: 'Flexible Timings', description: 'Morning, afternoon, and evening batches. Study after school hours.' },
  { icon: Shield, title: 'NCERT Aligned', description: 'Our curriculum bridges TN syllabus with NCERT for complete NEET coverage.' },
]
...
const faqs = [
  {
    ...
    answer: 'Tamil Nadu State Board Biology syllabus is approximately 70-75% aligned with NEET. The TN board follows Samacheer Kalvi curriculum which has good coverage but gaps exist in Biotechnology Applications, detailed Ecology, and some Genetics topics. Our coaching bridges these gaps with NCERT integration.',
  },
  {
    question: 'Which cities in Tamil Nadu do you serve?',
    answer: 'We serve students from all Tamil Nadu cities including Chennai, Coimbatore, Madurai, Tiruchirappalli, Salem, Tirunelveli, Erode, Vellore, and more. Our online live classes are accessible from anywhere in Tamil Nadu.',
  },
  {
    question: 'What is the fee for TN Board Biology + NEET coaching?',
    answer: 'Our comprehensive TN Board + NEET Biology coaching ranges from Rs 24,000 to Rs 68,000 per year. This includes board exam preparation, NEET coaching, study materials, and unlimited doubt sessions. EMI options available.',
  },
  {
    question: 'Which medical colleges can Tamil Nadu students target?',
    answer: 'With strong NEET scores, TN students can target: Madras Medical College, Stanley Medical College, Kilpauk Medical College, Coimbatore Medical College, CMC Vellore, plus AIIMS, JIPMER Puducherry, and other all-India institutes.',
  },
  {
  ...
  },
  {
    ...
    answer: 'We follow an integrated approach: Cover all Samacheer Kalvi syllabus for board scoring while adding NCERT-specific topics. Special modules for Biotechnology Applications, detailed Ecology, and advanced Genetics ensure complete NEET preparation.',
  },
]

const premiumSchools = [
  'DAV Schools Chennai', 'PSBB Schools', 'Chettinad Vidyashram', 'Sishya School', 'Padma Seshadri Bala Bhavan',
  'National Public School Chennai', 'Vidya Mandir Schools', 'Bharatiya Vidya Bhavan', 'GD Matriculation',
  'Velammal Schools',
]
...
export default function TamilNaduBiologyTuitionPage() {
  const handleWhatsAppClick = () => {
    if (typeof window !== 'undefined' && (window as any).gtag) {
      ;(window as any).gtag('event', 'whatsapp_click', { event_category: 'conversion', event_label: 'tamil_nadu_biology_page' })
    }
  ...
  return (
    <div className="min-h-screen">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        name: 'Cerebrum Biology Academy - Tamil Nadu Biology Tuition',
        description: 'Best online biology tuition for Tamil Nadu State Board students preparing for NEET',
        ...
        <div className="relative max-w-7xl mx-auto px-4">
          <motion.div className="text-center max-w-4xl mx-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="inline-flex items-center bg-white/20 backdrop-blur-sm px-6 py-3 rounded-full text-sm font-medium mb-6">
              <MapPin className="w-5 h-5 mr-2" />Tamil Nadu State Board | Samacheer Kalvi + NEET
            </div>
            ...
            <h2 className="text-xl md:text-2xl opacity-90 mb-4">Chennai | Coimbatore | Madurai | Trichy | All TN Cities</h2>

            <p className="text-lg md:text-xl opacity-80 mb-8 max-w-3xl mx-auto">
              Expert TN Board Biology coaching with NEET focus. 94.2% success rate, AIIMS faculty. Bridge the
              gap between Samacheer Kalvi and NCERT syllabus. Join 1,500+ Tamil Nadu students!
            </p>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">Biology Coaching Across Tamil Nadu</h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">From Chennai to Madurai, Coimbatore to Trichy - premium online coaching for every TN student.</p>
          </motion.div>
                ...
                <div className={`bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-all hover:-translate-y-1 ${city.priority === 'high' ? 'ring-2 ring-[#1e3a5f]' : ''}`}>
                  ...
                  {city.priority === 'high' && (
                    <div className="mt-2 inline-flex items-center text-xs bg-[#1e3a5f]/10 text-[#1e3a5f] px-2 py-1 rounded-full">
                      <Star className="w-3 h-3 mr-1" />Top City
                    </div>
                  )}
                </div>
              </motion.div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">Tamil Nadu vs NEET Syllabus</h2></motion.div>
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-green-50 rounded-2xl p-8 border border-green-200">
              <h3 className="text-2xl font-bold text-green-800 mb-6 flex items-center"><CheckCircle className="w-6 h-6 mr-2" />Common Topics (70-75%)</h3>
              <ul className="space-y-3">
                {['Cell Biology & Biomolecules', 'Plant & Human Physiology', 'Reproduction Chapters', 'Genetics & Inheritance', 'Classification of Organisms', 'Structural Organisation', 'Neural Control'].map((topic, i) => (
                  <li key={i} className="flex items-center text-gray-700"><CheckCircle className="w-5 h-5 text-green-600 mr-2 flex-shrink-0" />{topic}</li>
                ))}
              </ul>
            </div>
            <div className="bg-red-50 rounded-2xl p-8 border border-red-200">
              <h3 className="text-2xl font-bold text-red-800 mb-6 flex items-center"><FileText className="w-6 h-6 mr-2" />Gap Topics (Need NCERT)</h3>
              <ul className="space-y-3">
                {['Biotechnology Applications Detail', 'Ecosystem & Environmental Issues', 'Biodiversity Conservation', 'Microbes in Human Welfare', 'Evolution Mechanisms', 'Human Health & Diseases', 'Organisms & Populations'].map((topic, i) => (
                  <li key={i} className="flex items-center text-gray-700"><ArrowRight className="w-5 h-5 text-red-600 mr-2 flex-shrink-0" />{topic}</li>
                ))}
              </ul>
            </div>
          </div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">Why Tamil Nadu Students Choose Us</h2></motion.div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          <div className="bg-[#1e3a5f]/5 rounded-2xl p-8">
            <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Students from These Tamil Nadu Schools Trust Us</h3>
            <div className="flex flex-wrap justify-center gap-4">
              {premiumSchools.map((school, index) => (
                <motion.span key={school} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4, delay: index * 0.05 }} viewport={{ once: true }} className="bg-white text-gray-700 px-4 py-2 rounded-full font-medium shadow-sm">{school}</motion.span>
              ))}
            </div>
          </div>
        </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4">
          <motion.div className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">FAQs - Tamil Nadu Biology Tuition</h2></motion.div>
      ...
      <section className="py-20 bg-gradient-to-r from-[#1e3a5f] via-[#2d5a87] to-[#3d7ab5] text-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-5xl font-bold mb-6">Ready to Excel in TN Board Biology & NEET?</h2>
            <p className="text-xl md:text-2xl mb-8 opacity-90">Join 1,500+ Tamil Nadu students. 94.2% success rate. AIIMS faculty. Start today!</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
              <Button variant="secondary" size="xl" onClick={handleWhatsAppClick} className="bg-[#25D366] text-white hover:bg-[#20BD5A] flex items-center justify-center gap-2">
                <MessageCircle className="w-6 h-6" /><span>WhatsApp Us Now</span>
              </Button>
            ...
            <div className="grid md:grid-cols-4 gap-6 max-w-3xl mx-auto text-sm mt-12">
              {['All TN Cities', 'Live Classes', 'Board + NEET', 'NCERT Aligned'].map((item) => (
                <div key={item} className="flex items-center justify-center"><CheckCircle className="w-4 h-4 mr-2" /><span>{item}</span></div>
              ))}
            </div>
          </motion.div>
        </div>
