    ...
    options: [
      ...
      },
      {
        ...
        pricing: [
          {
            tier: 'Ascent PCB',
            batch: '12–16 (Bio) | Focused (P&C)',
            fee: '₹2,28,000/yr',
            total: '₹4,56,000 (2yr)',
          },
          {
            tier: 'Pinnacle PCB',
            batch: 'Max 12 (Bio) | Small (P&C)',
            fee: '₹2,94,000/yr',
            total: '₹5,88,000 (2yr)',
          },
        ],
...
const ADDONS = [
  {
    ...
    fee: '₹12,000',
    icon: '📝',
    desc: 'Full NEET-pattern test series with OMR, analysis, ranking & error tracking',
  },
  {
    name: '1-on-1 Mentorship',
    fee: '₹1,50,000',
    icon: '🧑‍🏫',
    desc: 'Personal mentorship with Dr. Shekhar (AIIMS). Custom study plan, weekly reviews',
  },
  {
    name: 'SGP',
    fee: 'Discuss with us',
    icon: '⭐',
    desc: 'Special Guidance Programme — exclusive intensive, limited seats',
  },
]
...
export default function WhatsAppCourseSelector() {
  ...
  function handleLeadSubmit(e: FormEvent) {
    ...
    try {
      ...
      localStorage.setItem(
        'cerebrum_user',
        JSON.stringify({ name: leadName, phone: leadPhone, city: leadCity })
      )
    } catch {}
  ...
  function handleSkip() {
    try {
      localStorage.setItem('cerebrum_user', 'skipped')
    } catch {}
    setUserRegistered(true)
    setShowModal(false)
  ...
  const sty = {
    ...
    brandStrip: {
      height: 4,
      background: `linear-gradient(90deg, ${C.purple}, ${C.indigo}, ${C.blue}, ${C.green})`,
    } as React.CSSProperties,
    header: {
  ...
  return (
    <div style={sty.page}>
        ...
        {classKeys.map((cls) => {
          ...
          return (
            <button
              key={cls}
              onClick={() => {
                setSelectedClass(active ? null : cls)
                setExpandedCard(null)
              }}
            ...
            return (
              <div
                ...
                style={{
                  ...
                  boxShadow: isExp
                    ? `0 4px 20px ${rgba(opt.color, 0.12)}`
                    : isHov
              ...
              >
                ...
                <h3 style={{ fontSize: 15, fontWeight: 700, margin: '0 0 6px', lineHeight: 1.3 }}>
                  {opt.goal}
                </h3>
                <p style={{ fontSize: 12, color: C.g600, lineHeight: 1.5, margin: '0 0 14px' }}>
                  {opt.desc}
                </p>
                ...
                <div style={{ borderTop: `1px solid ${C.g100}`, paddingTop: 10, marginBottom: 6 }}>
                  {opt.pricing.map((p, j) => (
                    ...
                    >
                      <div>
                        <span style={{ fontSize: 11, color: C.g600, fontWeight: 600 }}>
                          {p.tier}
                        </span>
                        {!isExp && (
                          <span style={{ fontSize: 9, color: C.g400, marginLeft: 5 }}>
                            {p.batch}
                          </span>
                        )}
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <span style={{ fontSize: 14, fontWeight: 800, color: opt.color }}>
                          {p.fee}
                        </span>
                  ...
                  <div style={{ marginTop: 14 }} onClick={(e) => e.stopPropagation()}>
                    ...
                    <Section title="What's Included" color={opt.color}>
                      {opt.features.map((f, k) => (
                        ...
                        >
                          <span style={{ color: opt.color, flexShrink: 0, fontWeight: 700 }}>
                            ✓
                          </span>
                          <span>{f}</span>
                        </div>
                      ...
                      {CENTRES.map((c, k) => (
                        ...
                        >
                          <span style={{ color: C.dark }}>
                            {c.type === 'online' ? '💻' : '📍'} {c.name}
                          </span>
                          <span style={{ color: C.g600, fontSize: 10.5 }}>{c.loc}</span>
                        </div>
                      ))}
                      <div
                        style={{ fontSize: 9.5, color: C.g400, marginTop: 5, fontStyle: 'italic' }}
                      >
                        Same curriculum, same faculty, same results — regardless of mode.
                      </div>
                    </Section>
                      ...
                      {ADDONS.map((a, k) => (
                        <div
                          ...
                          style={{
                            ...
                            alignItems: 'center',
                            padding: '5px 0',
                            borderTop: k > 0 ? `1px solid ${C.g100}` : 'none',
                          }}
                        >
                          <div>
                            <span style={{ fontSize: 11.5, fontWeight: 600, color: C.dark }}>
                              {a.icon} {a.name}
                            </span>
                            <div style={{ fontSize: 9.5, color: C.g600, marginTop: 1 }}>
                              {a.desc}
                            </div>
                          </div>
                          <span
                    ...
                    >
                      Payment: 1st instalment on Day 1 after demo, 2nd on Day 45, 3rd on Day 61. GST
                      extra. Scholarship via Cerebrum Aptitude Test (CAT).
                    </div>
      ...
      <div style={sty.bottomCta}>
        ...
        <div style={{ marginTop: 8, fontSize: 13, fontWeight: 600, color: C.dark }}>
          📞 +91 88264 44334
        </div>
        <div style={{ marginTop: 4, fontSize: 10, color: C.g400 }}>
          CBSE • ICSE • IB • IGCSE • A-Levels
        </div>
      </div>
      ...
      <div style={sty.footer}>
        ...
        <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.5)', marginTop: 2 }}>
          Rohini • Gurugram • South Delhi • Online | www.cerebrumbiologyacademy.com
        </div>
      </div>
      ...
      {showModal && selectedClass && pendingCard && (
        <div style={sty.overlay}>
          <div style={sty.modal}>
            <h3 style={{ fontSize: 18, fontWeight: 800, color: C.dark, margin: '0 0 4px' }}>
              📋 Get Full Course Details
            </h3>
            <p style={{ fontSize: 12, color: C.g600, marginBottom: 12, lineHeight: 1.4 }}>
              Share your details to explore the complete program, fee structure, and book a FREE
              demo class.
            </p>
            <div
              style={{
                ...
                letterSpacing: 0.5,
                marginBottom: 14,
                background: rgba(COURSES[selectedClass].options[pendingCard.idx].color, 0.1),
                color: COURSES[selectedClass].options[pendingCard.idx].color,
              }}
            >
