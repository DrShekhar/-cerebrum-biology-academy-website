/**
 * Admin LMS — Chapter update/delete.
 * Deleting a chapter cascades to its topics (schema onDelete: Cascade).
 */
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const chapter = await prisma.chapters.update({ where: { id: params.id }, data })
...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await prisma.chapters.delete({ where: { id: params.id } })
