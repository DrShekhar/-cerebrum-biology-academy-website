...
async function getUserTopicPerformance(userId: string, freeUserId: string | null, topics: string[]) {
  try {
    ...
    return userProgress.reduce((acc, progress) => {
      acc[progress.topic] = {
...
function calculateAdaptiveDifficulty(topicPerformance: Record<string, any>, topic: string): string[] {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const userAnsweredQuestions = !params.includeAnswered ?
      await prisma.userQuestionResponse.findMany({
        where: {
    ...
    const questionsWithStats = selectedQuestions.map((q, index) => ({
      ...
      successRate: q.totalAttempts > 0 ?
        Math.round((q.correctAttempts / q.totalAttempts) * 100 * 100) / 100 : null,
      estimatedDifficulty: q.totalAttempts > 10 ?
    ...
    const topicDistribution = questionsWithStats.reduce((acc, q) => {
      acc[q.topic] = (acc[q.topic] || 0) + 1
      return acc
    }, {} as Record<string, number>)

    const difficultyDistribution = questionsWithStats.reduce((acc, q) => {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        statistics: {
          ...
          averageMarks: Math.round((questionsWithStats.reduce((sum, q) => sum + q.marks, 0) / questionsWithStats.length) * 100) / 100,
          averageSuccessRate: questionsWithStats.filter(q => q.successRate !== null).length > 0 ?
            Math.round((questionsWithStats.filter(q => q.successRate !== null).reduce((sum, q) => sum + (q.successRate || 0), 0) /
              questionsWithStats.filter(q => q.successRate !== null).length) * 100) / 100 : null
        },
        userContext: params.adaptiveMode || params.weightByPerformance ? {
          topicPerformance,
          adaptiveDifficulties: params.topics ?
            params.topics.reduce((acc, topic) => {
