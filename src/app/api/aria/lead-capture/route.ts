...
export async function POST(request: NextRequest) {
  try {
    ...
    // Find counselor for assignment (required field)
    ...
    // Fallback: get any admin user if no counselor found
    const assignee =
      counselor ||
    ...
    if (!assignee) {
      logger.error('No counselor or admin found to assign lead', { service: 'aria-lead' })
    ...
    const newLead = await prisma.leads.create({
      data: {
        ...
        source: 'WEBSITE', // Chatbot is on website
...
⚡ CALL WITHIN 30 MINUTES for best conversion!`,
    ...
    // Log activity
    const activityId = `act_${Date.now()}_${Math.random().toString(36).substring(7)}`
