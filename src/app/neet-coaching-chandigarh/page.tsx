...
const WHATSAPP_MESSAGE = encodeURIComponent('Hi! I am interested in NEET coaching in Chandigarh. Please share details.')
...
const features = [
  { icon: Video, title: 'Live Interactive Classes', description: 'Real-time online coaching for Chandigarh Tricity. No commute needed.' },
  { icon: Users, title: 'Small Batches (10-15)', description: 'Personalized attention unlike crowded Sector 34 coaching centers.' },
  { icon: Award, title: 'AIIMS Trained Faculty', description: 'Expert doctors and teachers from premier medical institutions.' },
  { icon: BookOpen, title: 'CBSE + NEET Focus', description: 'Complete biology coaching for both board exams and NEET.' },
  { icon: Clock, title: 'Flexible Timings', description: 'Morning, afternoon, evening batches. Fit your school schedule.' },
  { icon: Shield, title: 'Tricity Coverage', description: 'Chandigarh, Panchkula, Mohali - all served with same quality.' },
]
...
const faqs = [
  {
    ...
    answer: 'Chandigarh coaching centers in Sector 34 or 35 typically have 50-100+ students per batch. Our online classes limit to 10-15 students ensuring personalized attention. Plus, save 1-2 hours daily commute from Panchkula/Mohali. Same 94.2% success rate, better attention.',
  },
  {
    question: 'Which areas in Chandigarh Tricity do you serve?',
    answer: 'We serve all Chandigarh sectors (15, 17, 22, 35, 44 etc.), Panchkula (all sectors), Mohali (Phase 1-11, IT City), Zirakpur, Kharar, and surrounding areas. Online live classes mean you can join from anywhere in Tricity.',
  },
  {
    question: 'What is the fee for NEET coaching in Chandigarh?',
    answer: 'Our comprehensive NEET Biology coaching ranges from Rs 24,000 to Rs 68,000 per year depending on the program. This is comparable to Chandigarh coaching centers but with much smaller batches and personalized attention. EMI available.',
  },
  {
    question: 'Which medical colleges can Chandigarh students target?',
    answer: 'With strong NEET scores, Tricity students can target: PGIMER Chandigarh (top choice!), Government Medical College Chandigarh, GMCH Patiala, DMC Ludhiana, plus AIIMS Delhi/Bathinda, and other all-India institutes.',
  },
  {
    question: 'Do you prepare for both CBSE boards and NEET?',
    answer: 'Yes! Most Chandigarh students follow CBSE curriculum which aligns well with NEET. We cover both board exam preparation and NEET-specific training. Score 95+ in boards while preparing for NEET simultaneously.',
  },
  {
    question: 'How is your coaching different from Aakash/Allen in Chandigarh?',
    answer: 'Unlike large coaching chains with 100+ students per batch, we keep batches of 10-15 only. Our online format offers flexibility and saves commute time. AIIMS faculty, personalized attention, and 94.2% success rate make us the smart choice.',
  },
]

const premiumSchools = [
  'DAV Public Schools Chandigarh', 'St. Johns High School', 'Carmel Convent School', 'Bhavan Vidyalaya',
  'St. Kabir Public School', 'Strawberry Fields High', 'Vivek High School', 'Aksips Schools',
  'DPS Chandigarh', 'St. Annes Convent',
]
...
export default function NEETCoachingChandigarhPage() {
  const handleWhatsAppClick = () => {
    if (typeof window !== 'undefined' && (window as any).gtag) {
      ;(window as any).gtag('event', 'whatsapp_click', { event_category: 'conversion', event_label: 'chandigarh_neet_page' })
    }
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="relative max-w-7xl mx-auto px-4">
          <motion.div className="text-center max-w-4xl mx-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="inline-flex items-center bg-white/20 backdrop-blur-sm px-6 py-3 rounded-full text-sm font-medium mb-6">
              <MapPin className="w-5 h-5 mr-2" />The City Beautiful | Chandigarh Tricity
            </div>
            ...
            <h2 className="text-xl md:text-2xl opacity-90 mb-4">Sector 17 | Sector 35 | Panchkula | Mohali | All Tricity</h2>

            <p className="text-lg md:text-xl opacity-80 mb-8 max-w-3xl mx-auto">
              Premium NEET Biology coaching for Chandigarh Tricity. 94.2% success rate, AIIMS faculty.
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div className="text-center mb-16">
            <h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">NEET Coaching Across Chandigarh Tricity</h2>
          </motion.div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {chandigarhLocalities.map((locality, index) => (
              <motion.div key={locality.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: index * 0.05 }} viewport={{ once: true }}>
                <div className={`bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-all hover:-translate-y-1 ${locality.priority === 'high' ? 'ring-2 ring-[#1e3a5f]' : ''}`}>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">Why Chandigarh Students Choose Us</h2></motion.div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature, index) => (
              <motion.div key={feature.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: index * 0.1 }} viewport={{ once: true }} className="bg-gray-50 rounded-xl p-8 shadow-lg">
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="bg-white rounded-2xl p-8 shadow-lg">
            <h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Students from These Chandigarh Schools Trust Us</h3>
            <div className="flex flex-wrap justify-center gap-4">
              {premiumSchools.map((school) => (
                <span key={school} className="bg-gray-100 text-gray-700 px-4 py-2 rounded-full font-medium">{school}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4">
          <motion.div className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">FAQs - NEET Coaching Chandigarh</h2></motion.div>
      ...
      <section className="py-20 bg-gradient-to-r from-[#1e3a5f] via-[#2d5a87] to-[#3d7ab5] text-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-5xl font-bold mb-6">Ready to Crack NEET from Chandigarh?</h2>
            <p className="text-xl md:text-2xl mb-8 opacity-90">Join 780+ Tricity students. Target PGIMER, GMCH. 94.2% success rate!</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
              <Button variant="secondary" size="xl" onClick={handleWhatsAppClick} className="bg-[#25D366] text-white hover:bg-[#20BD5A] flex items-center justify-center gap-2">
                <MessageCircle className="w-6 h-6" /><span>WhatsApp Us Now</span>
              </Button>
