...
function getMetaDescriptionByType(area: ReturnType<typeof getAreaBySlug>): string {
  ...
  switch (area.type) {
    case 'coaching-hub':
      return `Top NEET Biology coaching in ${area.name}, North Delhi - coaching hub. ${CEREBRUM_METRICS.successRateText} success rate, AIIMS faculty. Students from ${schoolsText}. Near ${metroText}. Small batches, expert guidance. Book free demo!`

    case 'posh':
      return `Premium NEET coaching in ${area.name}, North Delhi for educated families. ${CEREBRUM_METRICS.successRateText} success rate. Serving ${schoolsText} students. Near ${metroText}. Personalized attention, flexible timings. Join now!`

    case 'commercial':
      return `Best NEET Biology coaching near ${area.name}, North Delhi. ${CEREBRUM_METRICS.successRateText} success rate, AIIMS faculty. Students from ${schoolsText}. Near ${metroText}. Small batches, proven results. Book free demo!`

    case 'residential':
    default:
      return `Best NEET Biology coaching near ${area.name}, North Delhi. ${CEREBRUM_METRICS.successRateText} success rate, AIIMS faculty. Students from ${schoolsText}. Near ${metroText}. Small batches, personal mentorship. Book free demo!`
  }
}
...
function getTitleByType(area: ReturnType<typeof getAreaBySlug>): string {
  ...
  return `${prefix} NEET Coaching in ${area.name}, North Delhi | ${CEREBRUM_METRICS.successRateText} Success | Cerebrum Academy`
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    keywords: [
      ...
      `NEET preparation ${area.name} North Delhi`,
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `NEET Coaching in ${area.name}, North Delhi`,
        },
      ],
    },
    twitter: {
    ...
    alternates: {
      canonical: `https://cerebrumbiologyacademy.com/neet-coaching-north-delhi/${areaSlug}`,
    },
    robots: {
      index: true,
      follow: true,
...
export default async function NorthDelhiAreaPage({ params }: PageProps) {
  const { area: areaSlug } = await params
  const area = getAreaBySlug(areaSlug)
  ...
  const localBusinessId = `https://cerebrumbiologyacademy.com/neet-coaching-north-delhi/${areaSlug}#localbusiness`
  ...
  const getAreaReviews = () => {
    ...
    return [
      ...
      },
      {
        ...
        reviewRating: { '@type': 'Rating', ratingValue: '5', bestRating: '5' },
        reviewBody: `Best Biology coaching for North Delhi students. Dr. Shekhar Sir's teaching methodology made complex topics easy. The ${area.nearbyMetro[0] || 'metro'} connectivity makes it convenient to reach.`,
      },
      {
        '@type': 'Review',
        itemReviewed,
  ...
  const educationalOrgSchema = {
    ...
    description: `Best NEET coaching in ${area.name}, North Delhi. ${area.description}`,
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        '@type': 'ListItem',
        position: 2,
        name: 'NEET Coaching North Delhi',
      ...
      },
      {
        ...
        item: `https://cerebrumbiologyacademy.com/neet-coaching-north-delhi/${areaSlug}`,
      },
    ],
  }
  ...
  const faqSchema = {
    ...
    mainEntity: [
      {
        '@type': 'Question',
        name: `What is the best NEET coaching in ${area.name}, North Delhi?`,
        acceptedAnswer: {
          '@type': 'Answer',
      ...
      },
      {
        ...
        acceptedAnswer: {
          '@type': 'Answer',
          text: `Students from ${area.name} can take Yellow Line from ${area.nearbyMetro[0]} to Green Park or AIIMS. Our center in Kalu Sarai is just 5 minutes from Green Park Metro.`,
        },
      },
      {
        '@type': 'Question',
