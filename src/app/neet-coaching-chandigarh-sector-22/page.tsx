...
import { Users, Trophy, Star, Award, Video, MessageCircle, ArrowRight, GraduationCap, Phone, Target, Laptop, School, BookOpen, Clock } from 'lucide-react'
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {successMetrics.map((m, i) => (
              <motion.div key={m.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="text-center p-6 rounded-xl bg-emerald-50">
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
            {nearbyAreas.map((a, i) => (
              <motion.div key={a.name} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: i * 0.05 }} className={`p-4 rounded-xl text-center ${a.priority === 'high' ? 'bg-emerald-600 text-white' : 'bg-white'}`}>
