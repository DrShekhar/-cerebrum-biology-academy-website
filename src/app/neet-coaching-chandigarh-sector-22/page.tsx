...
const WHATSAPP_MESSAGE = encodeURIComponent(
  'Hi! I am interested in NEET coaching (Sector 22 Chandigarh). Please share details.'
)
...
const features = [
  {
    ...
    description: 'Study from Sector 22 home. Skip traffic to Sector 34.',
  },
  {
    icon: Users,
    title: 'Small Batches (10-15)',
    description: 'Personal attention unlike crowded coaching centers.',
  },
...
const schools = [
  'Government Model School Sector 20',
  'DAV Sector 8',
...
const faqs = [
  {
    question: 'Which is the best NEET coaching near Sector 22 Chandigarh?',
    answer:
      'Cerebrum Biology Academy offers excellent NEET coaching for Sector 22 students with AIIMS faculty, 98% success rate. 95+ students from Commercial Hub area enrolled. Online classes - no Sector 34 traffic!',
  },
  {
    question: 'How much does NEET coaching cost for Sector 22 students?',
    answer:
      'Cerebrum fees: Foundation Rs 24,000/year, Comprehensive Rs 36,000/year, Dropper Rs 68,000/year. Save 60-70% compared to Sector 34 coaching centers.',
  },
  {
    question: 'What are batch timings for Sector 22 students?',
    answer:
      'Multiple options: Morning (6-8 AM), After-school (4-6 PM), Evening (7-9 PM). Weekend batches also available.',
  },
  {
    question: 'Is dropper batch available?',
    answer:
      'Yes! Intensive dropper batch at Rs 68,000/year with 6-8 hours daily coaching, personal mentorship, weekly tests.',
  },
  {
    question: 'How do students clear doubts online?',
    answer:
      'Live doubt solving, WhatsApp to faculty, weekly doubt sessions, one-on-one video calls. Every doubt answered!',
  },
  {
    question: 'How to book free demo from Sector 22?',
    answer:
...
export default function Sector22Page() {
  return (
    <div className="min-h-screen">
      <CitySchema
        cityName="Sector 22 Chandigarh"
        citySlug="chandigarh-sector-22"
        state="Chandigarh"
        localities={nearbyAreas.map((a) => a.name)}
        faqs={faqs}
        studentCount="95+"
        coordinates={{ lat: '30.7333', lng: '76.7694' }}
      />
        ...
        <div className="container mx-auto px-4 text-center">
          <div className="animate-fadeInUp">
            <span className="inline-block px-4 py-2 bg-emerald-500/20 text-emerald-300 rounded-full text-sm font-medium mb-6">
              🎯 95+ Students from Commercial Hub Area
            </span>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {successMetrics.map((m, i) => (
              <div
                key={m.label}
                className="text-center p-6 rounded-xl bg-emerald-50 animate-fadeInUp"
              >
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-12">
            Students from <span className="text-emerald-600">Sector 22 Area</span>
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
            {nearbyAreas.map((a, i) => (
              <div
                key={a.name}
                className={`p-4 rounded-xl text-center ${a.priority === 'high' ? 'bg-emerald-600 text-white' : 'bg-white'}`}
              >
                <div className="font-bold">{a.name}</div>
                <div className={a.priority === 'high' ? 'text-emerald-100' : 'text-gray-600'}>
                  {a.students} students
                </div>
              </div>
            ))}
      ...
      <section className="py-16 bg-emerald-50">
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-8">
            <School className="w-8 h-8 inline mr-2 text-emerald-600" />
            Nearby Schools
          </h2>
          <div className="flex flex-wrap justify-center gap-3">
            {schools.map((s) => (
              <span key={s} className="px-4 py-2 bg-white text-emerald-700 rounded-full text-sm">
                {s}
              </span>
            ))}
          </div>
