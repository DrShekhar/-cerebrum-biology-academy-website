...
const WHATSAPP_MESSAGE = encodeURIComponent('Hi! I am interested in NEET coaching in Secunderabad Hyderabad. Please share details.')
...
const features = [
  { icon: Video, title: 'Live Interactive Classes', description: 'Premium online coaching - no Secunderabad-Hyderabad commute. World-class teaching from home.' },
  { icon: Users, title: 'Elite Small Batches (10-15)', description: 'Exclusive batches for Secunderabad students with personalized attention.' },
  { icon: Award, title: 'AIIMS Trained Faculty', description: 'Expert doctors and teachers from premier medical institutions.' },
  { icon: BookOpen, title: 'Telangana & CBSE Expert', description: 'Complete coverage of TSBIE Inter syllabus and CBSE for NEET.' },
  { icon: Clock, title: 'Flexible Timings', description: 'Morning, afternoon, evening batches to complement school schedules.' },
  { icon: Shield, title: 'Zero Commute Stress', description: 'No traffic to coaching hubs in Ameerpet. Learn from your residence.' },
]
...
const faqs = [
  { question: 'Why do Secunderabad students choose online NEET coaching?', answer: 'Secunderabad has excellent schools like Pallavi Model School, Delhi Public School, and St. Ann\'s with demanding schedules. Our online classes offer flexibility that physical coaching in Ameerpet cannot. Save 2-3 hours daily on traffic. Our 94.2% success rate proves online is equally effective.' },
  { question: 'Which areas in Secunderabad do you serve?', answer: 'We serve all Secunderabad localities including Secunderabad Main, Begumpet, Trimulgherry, Alwal, Malkajgiri, Tarnaka, Marredpally, Bowenpally, and all surrounding areas. Students from anywhere in Twin Cities can join our online live classes.' },
  { question: 'What is the fee for premium NEET coaching in Secunderabad?', answer: 'Our complete NEET Biology course ranges from Rs 24,000 to Rs 68,000 per year depending on the program chosen. This is competitive with Ameerpet coaching centers while offering superior flexibility and smaller batches. EMI options available.' },
  { question: 'How does this compare to coaching centers in Ameerpet or Kukatpally?', answer: 'Unlike physical coaching centers where batches have 50-100 students, we limit batches to 10-15 students. This means every doubt gets addressed, every student gets personal attention. Plus, you save hours on Hyderabad traffic daily.' },
  { question: 'What medical colleges can Secunderabad students target?', answer: 'With proper NEET preparation, Secunderabad students can target top colleges like Gandhi Medical College, Osmania Medical College, Deccan College of Medical Sciences, ESIC Medical College, and all-India institutes like AIIMS and JIPMER.' },
  { question: 'Do you cover Telangana Inter (TSBIE) syllabus?', answer: 'Yes! We cater to both Telangana State Board (TSBIE Inter) and CBSE students. Our program bridges TSBIE syllabus with NCERT for complete NEET preparation. 75-80% syllabus overlap means focused gap filling.' },
]

const premiumSchools = ['Pallavi Model School', 'Delhi Public School Secunderabad', 'St. Anns High School', 'Bharatiya Vidya Bhavan', 'Glendale Academy', 'Rosary Convent School', 'St. Patricks High School', 'Silver Oaks International', 'Oakridge International', 'Meridian School']
...
const whySecunderabad = [
  { icon: Sparkles, title: 'Premium Quality, Zero Commute', description: 'Get world-class NEET coaching without traveling to Ameerpet. No traffic hassle to coaching hubs.' },
  { icon: Building, title: 'Twin Cities Expert', description: 'Same faculty that teaches Jubilee Hills & Gachibowli elite. Now accessible from Secunderabad.' },
  { icon: GraduationCap, title: 'Defence Family Friendly', description: 'Flexible timings that work with defence and corporate families. Evening and weekend options.' },
]

export default function NeetCoachingSecunderabadPage() {
  const handleWhatsAppClick = () => {
    if (typeof window !== 'undefined' && (window as any).gtag) {
      ;(window as any).gtag('event', 'whatsapp_click', { event_category: 'conversion', event_label: 'secunderabad_neet_page' })
    }
  ...
  return (
    <div className="min-h-screen">
      <CitySchema cityName="Secunderabad" citySlug="secunderabad-hyderabad" state="Telangana" localities={localities.map((l) => l.name)} faqs={faqs} studentCount="850" coordinates={{ lat: '17.4399', lng: '78.4983' }} />
        ...
        <div className="relative max-w-7xl mx-auto px-4">
          <motion.div className="text-center max-w-4xl mx-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="inline-flex items-center bg-white/20 backdrop-blur-sm px-6 py-3 rounded-full text-sm font-medium mb-6">
              <MapPin className="w-5 h-5 mr-2" />Twin City | Premium NEET Coaching
            </div>
            <h1 className="text-4xl md:text-6xl font-bold mb-6">Best <span className="text-[#4ade80]">NEET Coaching in Secunderabad</span>, Hyderabad</h1>
            <h2 className="text-xl md:text-2xl opacity-90 mb-4">Begumpet | Trimulgherry | Alwal | Malkajgiri | Tarnaka</h2>
            <p className="text-lg md:text-xl opacity-80 mb-8 max-w-3xl mx-auto">Premium NEET Biology coaching for Secunderabad&apos;s best schools. 94.2% success rate, AIIMS faculty, zero Ameerpet traffic. Join 850+ students from Pallavi, DPS, St. Ann&apos;s!</p>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">NEET Coaching Across Secunderabad</h2></motion.div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">Why Secunderabad Families Choose Cerebrum</h2></motion.div>
          <div className="grid md:grid-cols-3 gap-8 mb-16">
            {whySecunderabad.map((item, index) => (<motion.div key={item.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} viewport={{ once: true }} className="bg-gray-50 rounded-xl p-8 border border-gray-200"><item.icon className="w-12 h-12 text-[#1e3a5f] mb-4" /><h3 className="text-xl font-bold text-gray-900 mb-2">{item.title}</h3><p className="text-gray-600">{item.description}</p></motion.div>))}
          </div>
          <div className="bg-gray-50 rounded-2xl p-8"><h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Students from These Secunderabad Schools Trust Us</h3><div className="flex flex-wrap justify-center gap-4">{premiumSchools.map((school) => (<span key={school} className="bg-white text-gray-700 px-4 py-2 rounded-full font-medium shadow-sm">{school}</span>))}</div></div>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4">
          <motion.div className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">FAQs - NEET Coaching Secunderabad</h2></motion.div>
      ...
      <section className="py-20 bg-gradient-to-r from-[#1e3a5f] via-[#2d5a87] to-[#3d7ab5] text-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}>
            <h2 className="text-3xl md:text-5xl font-bold mb-6">Join Secunderabad&apos;s Elite NEET Aspirants</h2>
            <p className="text-xl md:text-2xl mb-8 opacity-90">94.2% success rate, AIIMS faculty, premium small batches. No Ameerpet commute needed!</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
              <Button variant="secondary" size="xl" onClick={handleWhatsAppClick} className="bg-[#25D366] text-white hover:bg-[#20BD5A] flex items-center justify-center gap-2"><MessageCircle className="w-6 h-6" /><span>WhatsApp Us Now</span></Button>
