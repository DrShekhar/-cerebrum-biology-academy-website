...
const heroBlurb = "Lexington, Acton-Boxborough, Newton South, and Belmont consistently appear on USABO Semifinalist rosters. Boston's Route 128 academic belt — heavy with Indian-American and East-Asian-American families — is one of the densest USABO markets per capita. We run ET-evening classes calibrated for these schools' AP-heavy junior- and senior-year schedules."
const rigourBlurb = "Boston-area families come to Cerebrum because the local options are AP-tutoring shaped, not olympiad-shaped. Our AIIMS-trained, biology-only faculty teach to USABO Semifinal depth from week one, then layer on AP exam tactics in March–May. The Indian small-batch coaching tradition — faculty-led, weekly feedback, retrieval-heavy — produces results these families recognise."
const schools = [
  "Lexington HS",
...
const faqs = [
  {
    ...
    "answer": "Classes are 100% live online — no in-person centre in Massachusetts (Greater Boston metro). The live format means our AIIMS-trained faculty teach the same students from across the metro, scheduled for ET (Eastern) evenings (typically 7-9 PM your local time). Recordings are available if a student misses a session."
  },
  {
    "question": "Which Greater Boston schools do your students come from?",
    "answer": "We have repeat enrolments from the high-USABO-density schools in Massachusetts (Greater Boston metro) — listed above. We also coach students from any US high school; the schools listed are simply the ones where multiple students from the same school have trained with us."
  },
  {
