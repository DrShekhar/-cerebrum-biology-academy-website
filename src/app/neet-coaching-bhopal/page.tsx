...
import { MapPin, Users, Trophy, Star, CheckCircle, Award, BookOpen, Clock, Shield, Video, MessageCircle, Play, ArrowRight, GraduationCap, Building, Phone, Calculator, Target } from 'lucide-react'
...
const WHATSAPP_MESSAGE = encodeURIComponent('Hi! I am interested in NEET coaching in Bhopal. Please share details.')
...
const features = [
  { icon: Video, title: 'Live Interactive Classes', description: 'Real-time online coaching for Bhopal students. No commute to coaching centers.' },
  { icon: Users, title: 'Small Batches (10-15)', description: 'Personalized attention unlike crowded Bhopal coaching centers.' },
  { icon: Award, title: 'AIIMS Trained Faculty', description: 'Expert doctors and teachers from premier medical institutions.' },
  { icon: BookOpen, title: 'CBSE + MP Board Focus', description: 'Complete biology coaching for both board exams and NEET.' },
  { icon: Clock, title: 'Flexible Timings', description: 'Morning, afternoon, evening batches. Fit your school schedule.' },
  { icon: Shield, title: 'All Bhopal Covered', description: 'MP Nagar, Arera Colony, Kolar Road - all served with same quality.' },
]
...
const faqs = [
  { question: 'Why choose online NEET coaching over Bhopal coaching centers?', answer: 'Bhopal coaching centers typically have 50-100+ students per batch. Our online classes limit to 10-15 students ensuring personalized attention. Plus, save commute time from different parts of Bhopal. Same 94.2% success rate, better attention.' },
  { question: 'Which areas in Bhopal do you serve?', answer: 'We serve all Bhopal areas including MP Nagar, Arera Colony, Kolar Road, Hoshangabad Road, Shahpura, Ayodhya Nagar, TT Nagar, Bairagarh, and surrounding areas. Online live classes mean you can join from anywhere.' },
  { question: 'What is the fee for NEET coaching in Bhopal?', answer: 'Our comprehensive NEET Biology coaching ranges from Rs 24,000 to Rs 68,000 per year depending on the program. This is comparable to Bhopal coaching centers but with much smaller batches. EMI available.' },
  { question: 'Which medical colleges can Bhopal students target?', answer: 'With strong NEET scores, Bhopal students can target: Gandhi Medical College Bhopal, AIIMS Bhopal (top choice!), NSCB Medical College Jabalpur, MGM Medical College Indore, plus other all-India institutes.' },
  { question: 'Do you prepare for MP Board students?', answer: 'Yes! We cater to English medium students from both CBSE and MP Board. The biology syllabus overlap is significant, and we bridge any gaps with NCERT-focused teaching for NEET.' },
  { question: 'How do online classes work for practical biology?', answer: 'We use high-quality 3D animations, virtual lab simulations, and detailed diagrams. Our video explanations for practicals are often better than physical demonstrations. Plus, we provide practical notes and viva preparation.' },
]

const premiumSchools = ['Delhi Public School Bhopal', 'The Sanskaar Valley School', 'Campion School Bhopal', 'St. Josephs Convent', 'Carmel Convent School', 'Sagar Public School', 'Billabong High', 'Kendriya Vidyalaya Bhopal']
...
export default function NEETCoachingBhopalPage() {
  const handleWhatsAppClick = () => {
    if (typeof window !== 'undefined' && (window as any).gtag) {
      ;(window as any).gtag('event', 'whatsapp_click', { event_category: 'conversion', event_label: 'bhopal_neet_page' })
    }
  ...
  return (
    <div className="min-h-screen">
      <CitySchema cityName="Bhopal" citySlug="bhopal" state="Madhya Pradesh" localities={localities.map((l) => l.name)} faqs={faqs} studentCount="650" coordinates={{ lat: '23.2599', lng: '77.4126' }} />
        ...
        <div className="relative max-w-7xl mx-auto px-4">
          <motion.div className="text-center max-w-4xl mx-auto" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="inline-flex items-center bg-white/20 backdrop-blur-sm px-6 py-3 rounded-full text-sm font-medium mb-6">
              <MapPin className="w-5 h-5 mr-2" />City of Lakes | MP Capital
            </div>
            <h1 className="text-4xl md:text-6xl font-bold mb-6">Best <span className="text-[#4ade80]">NEET Coaching in Bhopal</span></h1>
            <h2 className="text-xl md:text-2xl opacity-90 mb-4">MP Nagar | Arera Colony | Kolar Road | All Bhopal</h2>
            <p className="text-lg md:text-xl opacity-80 mb-8 max-w-3xl mx-auto">Premium NEET Biology coaching for Bhopal students. 94.2% success rate, AIIMS faculty. Target AIIMS Bhopal, GMC with expert guidance. Join 650+ Bhopal students!</p>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">NEET Coaching Across Bhopal</h2></motion.div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          <motion.div className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">Why Bhopal Students Choose Us</h2></motion.div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature, index) => (<motion.div key={feature.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} viewport={{ once: true }} className="bg-gray-50 rounded-xl p-8 shadow-lg"><feature.icon className="w-12 h-12 text-[#1e3a5f] mb-4" /><h3 className="text-xl font-bold text-gray-900 mb-2">{feature.title}</h3><p className="text-gray-600">{feature.description}</p></motion.div>))}
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="bg-white rounded-2xl p-8 shadow-lg"><h3 className="text-2xl font-bold text-gray-900 mb-6 text-center">Students from These Bhopal Schools Trust Us</h3><div className="flex flex-wrap justify-center gap-4">{premiumSchools.map((school) => (<span key={school} className="bg-gray-100 text-gray-700 px-4 py-2 rounded-full font-medium">{school}</span>))}</div></div>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4">
          <motion.div className="text-center mb-16"><h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-6">FAQs - NEET Coaching Bhopal</h2></motion.div>
      ...
      <section className="py-20 bg-gradient-to-r from-[#1e3a5f] via-[#2d5a87] to-[#3d7ab5] text-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}>
            <h2 className="text-3xl md:text-5xl font-bold mb-6">Ready to Crack NEET from Bhopal?</h2>
            <p className="text-xl md:text-2xl mb-8 opacity-90">Join 650+ Bhopal students. Target AIIMS Bhopal, GMC. 94.2% success rate!</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
              <Button variant="secondary" size="xl" onClick={handleWhatsAppClick} className="bg-[#25D366] text-white hover:bg-[#20BD5A] flex items-center justify-center gap-2"><MessageCircle className="w-6 h-6" /><span>WhatsApp Us Now</span></Button>
