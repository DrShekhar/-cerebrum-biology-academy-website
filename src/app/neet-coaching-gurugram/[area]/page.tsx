...
function getMetaDescriptionByType(area: ReturnType<typeof getGurugramAreaBySlug>): string {
  ...
  switch (area.type) {
    case 'ultra-premium':
      return `Exclusive NEET Biology coaching for ${area.name} elite families. ${CEREBRUM_METRICS.successRateText} success rate. Students from ${schoolsText}. Residents of ${societiesText}. Same building as Aakash. Personalized ${CEREBRUM_METRICS.batchSizeText} batches. Book consultation!`
...
export default async function GurugramAreaPage({ params }: PageProps) {
  ...
  const eventSchema = {
    ...
    description: `Experience Cerebrum Biology Academy's teaching methodology. Free demo class for NEET aspirants from ${area.name}. Meet Dr. Shekhar C Singh at our Sector 51 center.`,
