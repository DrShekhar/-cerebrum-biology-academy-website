...
export const metadata: Metadata = {
  ...
  openGraph: {
    ...
    description:
      'NEET coaching fees in Noida range from Rs 45,000 to Rs 3,50,000. Compare all institutes.',
...
const quickAnswer = {
  ...
  answer:
    'NEET coaching fees in Noida range from Rs 45,000 to Rs 3,50,000 per year. Online-only courses start at Rs 25,000, while premium offline coaching at major institutes costs Rs 1,50,000-3,50,000. Cerebrum Academy offers specialized Biology coaching online from Rs 45,000/year with hybrid option at Rs 55,000/year.',
}
...
const faqs = [
  ...
  },
  {
    ...
    answer:
      'Not necessarily. Success depends on teaching quality, batch size, and your effort - not fees. Many students from Rs 45,000-65,000 programs score 650+ in NEET. Focus on faculty credentials and batch size over brand premium.',
  },
  {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-white">
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-4">
            NEET Coaching Fee Comparison Noida 2026
          </h2>
                ...
                {feeComparison.map((item, index) => (
                  ...
                  >
                    <td className="px-4 py-4">
                      ...
                      {item.highlight && (
                        <span className="text-xs text-green-600">Recommended for Biology</span>
                      )}
                    </td>
      ...
      <section className="py-16 bg-green-50">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl font-bold text-center mb-4">
            Cerebrum Academy Fee Options for Noida
          </h2>
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {cerebrumModes.map((item, index) => (
              <div
                key={index}
                className={`rounded-2xl p-6 ${item.recommended ? 'bg-yellow-400 relative' : 'bg-white border-2 border-gray-200'}`}
              >
                {item.recommended && (
                  <span className="absolute top-0 right-0 bg-green-800 text-white text-xs font-bold px-3 py-1 rounded-bl-lg">
                    RECOMMENDED
                  </span>
                )}
                <item.icon
                  className={`w-10 h-10 mb-4 ${item.recommended ? 'text-green-800' : 'text-green-600'}`}
                />
                ...
                <ul className="space-y-2 text-sm mb-4">
                  {item.features.map((f, i) => (
                    <li key={i} className="flex items-center gap-2">
                      <CheckCircle
                        className={`w-4 h-4 ${item.recommended ? 'text-green-800' : 'text-green-600'}`}
                      />
                      {f}
                    </li>
                  ))}
                </ul>
                <p className={`text-sm ${item.recommended ? 'text-green-900' : 'text-gray-500'}`}>
                  Best for: {item.best}
                </p>
              </div>
            ))}
      ...
      <section className="py-16 bg-green-800 text-white">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-xl text-green-100 mb-8">
            Specialized Biology coaching at affordable fees
          </p>
