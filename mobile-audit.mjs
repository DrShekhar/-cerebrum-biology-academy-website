...
async function testPage(browser, pageInfo) {
  ...
  try {
    ...
    const burgerMenu = await checkElement(page, '[data-testid="burger-menu"], .burger-menu, button[aria-label*="menu" i]', 'Burger Menu Button');
    ...
    if (pageInfo.name === 'homepage') {
      ...
      const heroHeading = await checkElement(page, 'h1, [class*="hero" i] h1, [class*="hero" i] h2', 'Hero Heading');
      const heroCTA = await checkElement(page, '[class*="hero" i] button, [class*="hero" i] a[class*="button" i]', 'Hero CTA Button');
    ...
    if (pageInfo.name === 'blog-list') {
      ...
      const leadForm = await checkElement(page, 'form, [class*="lead" i] form, [class*="capture" i] form, [class*="subscribe" i] form', 'Lead Capture Form');
...
async function generateMarkdownReport(reports) {
  ...
  const errorIssues = reports.reduce((sum, r) => sum + r.issues.filter(i => i.severity === 'error').length, 0);
  const warningIssues = reports.reduce((sum, r) => sum + r.issues.filter(i => i.severity === 'warning').length, 0);
