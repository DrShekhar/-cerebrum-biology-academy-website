...
export async function auditDependencies(params) {
        ...
        try {
            ...
            if (auditResult.metadata?.vulnerabilities) {
                ...
                if (total === 0) {
                    ...
                    if (vulns.moderate > 0)
                        output += `| 🟡 Moderate | ${vulns.moderate} |\n`;
                    if (vulns.low > 0)
                        output += `| 🟢 Low | ${vulns.low} |\n`;
                    output += '\n';
                    // List critical and high vulnerabilities
                    if (auditResult.vulnerabilities) {
                        const criticalVulns = Object.entries(auditResult.vulnerabilities).filter(([, v]) => v.severity === 'critical' || v.severity === 'high');
                        if (criticalVulns.length > 0) {
                            output += '**Critical/High Issues:**\n';
                            for (const [pkg, data] of criticalVulns.slice(0, 5)) {
        ...
        try {
            ...
            if (stdout.trim()) {
                ...
                if (packages.length === 0) {
                    ...
                    for (const [pkg, data] of sorted.slice(0, 15)) {
