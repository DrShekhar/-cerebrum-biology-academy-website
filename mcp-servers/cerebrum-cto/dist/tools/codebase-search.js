...
export async function searchCodebase(params) {
    ...
    try {
        ...
        const { stdout } = await execAsync(`grep -rn ${includePatterns} --exclude-dir=node_modules --exclude-dir=.next --exclude-dir=dist "${query}" ${projectRoot}/src 2>/dev/null | head -${maxResults * 2}`, { maxBuffer: 1024 * 1024 });
        // Parse results
        const results = [];
