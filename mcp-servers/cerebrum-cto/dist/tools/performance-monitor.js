...
export async function checkPerformance(params) {
    ...
    if (type === 'bundle' || type === 'both') {
        ...
        try {
            ...
            try {
                const { stdout } = await execAsync(`find .next/static -name "*.js" -size +100k -exec ls -lh {} \\; 2>/dev/null | head -10`, { cwd: projectRoot });
                if (stdout.trim()) {
