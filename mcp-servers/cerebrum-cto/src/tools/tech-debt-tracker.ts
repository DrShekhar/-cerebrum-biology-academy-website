...
export async function listTechDebt(params: ListParams) {
  ...
  try {
    ...
    for (let i = 1; i < sections.length; i += 2) {
      ...
      let itemPriority: TechDebtItem['priority'] = 'medium'
    ...
    for (const [prio, prioItems] of Object.entries(byPriority)) {
      if (prioItems.length === 0) continue
      ...
      for (const item of prioItems) {
        ...
        if (item.effort) output += ` (${item.effort})`
        output += '\n'
      }
      output += '\n'
    }
...
export async function addTechDebt(params: AddParams) {
  ...
  try {
    ...
    const prioritySections: Record<string, string> = {
      critical: '## 🔴 Critical',
      high: '## 🟠 High Priority',
