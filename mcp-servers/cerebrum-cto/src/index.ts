    ...
    tools: [
      {
        ...
        description: 'Semantic search across the Cerebrum codebase. Use for finding relevant code, patterns, or implementations.',
        inputSchema: {
          type: 'object',
          properties: {
            query: {
              type: 'string',
              description: 'Natural language search query (e.g., "authentication logic", "payment processing")',
            },
            fileTypes: {
              type: 'array',
              items: { type: 'string' },
      ...
      },
      {
        ...
        description: 'Audit project dependencies for outdated packages and security vulnerabilities.',
        inputSchema: {
          type: 'object',
          properties: {
            checkSecurity: {
  ...
  try {
    switch (name) {
      case 'codebase_search':
        return await searchCodebase(args as { query: string; fileTypes?: string[]; maxResults?: number })

      case 'tech_debt_list':
        return await listTechDebt(args as { priority?: string; status?: string })

      case 'tech_debt_add':
        return await addTechDebt(args as {
          title: string
          description?: string
          priority: string
          effort?: string
      ...
      case 'dependency_audit':
        return await auditDependencies(args as { checkSecurity?: boolean; checkOutdated?: boolean })

      case 'performance_check':
        return await checkPerformance(args as { url?: string; type?: 'lighthouse' | 'bundle' | 'both' })

      case 'memory_read':
        return await readMemory(args as { file: 'decisions' | 'learnings' | 'roadmap' | 'tech-debt' | 'all' })

      case 'memory_update':
        return await updateMemory(args as { file: 'decisions' | 'learnings' | 'roadmap' | 'tech-debt'; content: string })
