...
async function test() {
  ...
  for (const page of publicPages) {
    ...
    try {
      const res = await p.goto(baseUrl + page.path, { waitUntil: 'domcontentloaded', timeout: 20000 });
      ...
      results.push({ name: page.name, path: page.path, status, overflow, height, issues, ok: issues.length === 0 });
      console.log((issues.length === 0 ? '✅' : '❌') + ' ' + page.name + ' (' + page.path + ')' + (issues.length ? ' - ' + issues.join(', ') : ''));
    } catch (e) {
