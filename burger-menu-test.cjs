...
;(async () => {
  ...
  const burgerButton = await page
    .locator(
      'button[aria-label*="menu"], button[aria-label*="Menu"], [class*="burger"], [class*="hamburger"], button:has(svg)'
    )
    .first()
  ...
  if (await burgerButton.isVisible()) {
    ...
    const allTextInMenu = await page.evaluate(() => {
      const menu = document.querySelector(
        '[class*="mobile-menu"], [class*="MobileNav"], [class*="nav-menu"], [role="dialog"]'
      )
      if (menu) {
        return {
    ...
    const overlappingCheck = await page.evaluate(() => {
      const elements = document.querySelectorAll(
        'nav, [class*="nav"], [class*="header"], [class*="Navigation"]'
      )
      const results = []
      elements.forEach((el) => {
