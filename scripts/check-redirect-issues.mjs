...
const uncommented = redirectSection
  .split('\n')
...
const all = [
  ...
  ...[
    ...
    ...(cannibalizationConsolidationRedirects || []),
    ...(areaConsolidationRedirects || []),
  ].map((r) => ({ s: r.source, d: r.destination })),
]
...
for (const r of all) {
  if (r.d.endsWith('/') && r.d !== '/') {
    console.log('  TRAILING SLASH:', r.s, '->', r.d)
    found3++
  }
  if (r.d.includes('//')) {
    console.log('  DOUBLE SLASH:', r.s, '->', r.d)
    found3++
  }
  if (!r.d.startsWith('/') && !r.d.startsWith('http')) {
    console.log('  NO LEADING SLASH:', r.s, '->', r.d)
    found3++
  }
}
