 ...
 *   or
 *   tsx scripts/generate-seo-slugs.ts
 *
 * Output:
...
async function extractSlugsFromFile(
  ...
  try {
    ...
    console.log(`✓ Extracted ${slugs.length} slugs from ${categoryName}`)
    return slugs
  } catch (error) {
    console.error(`✗ Error reading ${categoryName}:`, error)
    return []
  }
}

async function generateSlugsFile(): Promise<void> {
  console.log('🔍 Extracting SEO page slugs...\n')
  ...
  const slugsByCategory: SlugsByCategory = {}
  let totalSlugs = 0
  ...
  console.log(`\n✅ Total slugs extracted: ${totalSlugs}\n`)
 ...
 *
 * MAINTENANCE:
 * - This file is AUTO-GENERATED by scripts/generate-seo-slugs.ts
 * - DO NOT edit manually
 * - Run \`npm run generate:seo-slugs\` after adding/removing SEO pages
 * - Last generated: ${today}
 * - Total pages: ${totalSlugs}
 *
 * @see scripts/generate-seo-slugs.ts
 */
...
generateSlugsFile().catch((error) => {
  console.error('❌ Error generating slugs file:', error)
  process.exit(1)
})
