...
const processFile = (filePath) => {
  ...
  try {
    ...
    content = content.replace(/console\.error\((.*?)\)/gs, (match, args) => {
    ...
    content = content.replace(/console\.warn\((.*?)\)/gs, (match, args) => {
      if (
        args.trim().startsWith("'") ||
        args.trim().startsWith('"') ||
        args.trim().startsWith('`')
...
files.forEach((file, index) => {
  process.stdout.write(
    `\rProcessing (${index + 1}/${files.length}): ${file.padEnd(80).substring(0, 80)}`
  )
