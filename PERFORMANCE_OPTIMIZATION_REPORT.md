...
const connectionPool = globalForPrisma.connectionPool ?? new Pool({
...
class OptimizedRedisCache {
  ...
  private getQuestionTTL(difficulty: string): number {
    switch (difficulty.toLowerCase()) {
      case 'easy': return 7200 // 2 hours
      case 'medium': return 3600 // 1 hour
      case 'hard': return 1800 // 30 minutes
      default: return 3600
    }
  }
  ...
  return async (request: NextRequest, context?: any) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        await optimizedCache.set(`api:${requestHash}`, {
          data: responseData,
...
export class QueryOptimizer {
  ...
  async getStudentEnrollments(userId: string, options: {
...
export class PerformanceMonitor extends EventEmitter {
  ...
  private analyzeDatabaseQueries(cutoff: number): any {
    ...
    for (const [queryType, queries] of this.metrics.database) {
      ...
      analytics.queryTypeStats.set(queryType, {
        ...
        p95QueryTime: this.calculatePercentile(recentQueries.map(q => q.duration), 95),
  ...
  private generateAlerts(requestAnalytics: any, databaseAnalytics: any): string[] {
    ...
    const cacheHitRate = this.metrics.cache.totalRequests > 0 ?
      (this.metrics.cache.hits / this.metrics.cache.totalRequests) * 100 : 100
