...
- **P3 Staff+recordings**: counselor academics endpoint+tab; admin drill-down; Zoom recording webhook → CF pipeline; optional Zoom auto-create.
  - **Zoom multi-class execution plan: `ZOOM_MULTI_CLASS_PLAN.md` (Jul 7)** — Pro license per simultaneous teacher (₹1,376/license/mo; reassignable between shifts); Video SDK rejected (30× cost). TWO code fixes required with it: (a) zoomService.ts:203,210 silently falls back to SIMULATED meetings w/ fake join URLs on API failure — must throw loudly for class sessions or students get dead links; (b) recordings webhook should DELETE the Zoom cloud recording after successful CF copy (5GB/license pooled storage fills in days) + set account auto-delete 7-14d backstop.
